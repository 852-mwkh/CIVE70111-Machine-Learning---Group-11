...
def print_iteration_panel(run_id, model_name, thr, f1, features_used, importance_dict):
    print("\n" + "="*60)
    print(f"ITERATION PANEL — RUN {run_id}")
    print("="*60)
    ...
    print("   • Chronological (time-based) — prevents future leakage")
    print("   • Test set is final chronological window")

    print("\n⚙️ PREDICTION BEHAVIOUR:")
    print("   • Predicts inverter state:")
    print("       Optimal     → 0")
    print("       Suboptimal → 1")
    print(f"   • Decision threshold used: {thr:.4f}")

    print("\n📊 PERFORMANCE (Suboptimal-Focused):")
    print(f"   • F1-Score (Suboptimal class = 1): {f1:.4f}")

    print("\n🧹 DATA PIPELINE APPLIED:")
    print("   • Weather cleaning (irradiation day/night logic)")
    print("   • AC/DC cleaning with interpolation")
    print("   • Yield monotonicity enforcement")
    print("   • Outlier removal using linear-regression residuals")
    print("   • Feature engineering: AC/IRRA, DC/IRRA, temporal ordering")
    ...
    print("   • ALE (Accumulated Local Effects) computed for all features")
    print("   • Drop-Column Importance:")
    for col, imp in importance_dict.items():
        print(f"       {col:25s} → importance = {imp:.4f}")
...
def run_classification_on_df_ps(df_ps_dict, test_days=10, val_days=3, drop_col=None):
    """
    Full pipeline + save-only plotting + unique filenames per run.
    """
    global run_count
    run_count += 1   # increment run ID
    ...
    # ================================================================
    print("\n=== Saving ALE plots ===")
    for feat in tqdm(X_te_model.columns):
        plot_ale_1d(svm, X_te_model, feat, save_path=folder_ale)
...
###################################################################################################
    print_iteration_panel(
        run_id=run_count,
        model_name="Linear SVM (Scaled Pipeline)",
        thr=thr_svm,
        f1=baseline_f1,
        features_used=X_te_model.columns,
        importance_dict=svm_importance
    )
...
###################################################################################################
    ...
    plt.show()
    plt.close()
    print("Displayed SVM histogram (not saved).")

    # ================================================================
    # SAVE RESULTS (PKL) — unique filename
    # ================================================================
    ...
    print("Results not saved (display only mode).")
...
drop1 = drop_base + ['AC/IRRA', 'DC/IRRA', 'MODULE_TEMPERATURE',
                     'TOTAL_YIELD_CLEAN', 'DC_CLEAN', 'AC_CLEAN']
print("\n\n==============================")
...
drop2 = drop_base + ['AC/IRRA', 'DC/IRRA', 'MODULE_TEMPERATURE',
                     'TOTAL_YIELD_CLEAN', 'DC_CLEAN', 'AC_CLEAN']
print("\n\n==============================")
...
drop3 = drop_base + ['DAILY_YIELD_CLEAN', 'AMBIENT_TEMPERATURE',
                     'MODULE_TEMPERATURE', 'AC_CLEAN',
                     'TOTAL_YIELD_CLEAN', 'DC/IRRA']
print("\n\n==============================")
print("PLANT 2: FEATURE-SELECTED MODEL (WITH OUTLIERS)")
print("==============================")
